import darkTokens from "@daikin-oss/dds-tokens/json/daikin/Dark/tokens.json";
import baseTokens from "@daikin-oss/dds-tokens/json/daikin/Light/tokens.json";
...
let html = await readFile(
  fromProjectDir("scripts/tokens-table-template.html"),
  "utf-8"
);
html = html.replaceAll("{{ DDS_TOKENS_VERSION }}", tokensVersion);
html = html.replaceAll("{{ TOKENS_TABLE_BODY }}", table);
await writeFile(fromProjectDir("scripts/generated.html"), html, "utf-8");
