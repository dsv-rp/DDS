...
@customElement("daikin-tile")
export class DaikinTile extends LitElement {
  ...
  // get daikin-tile-title from shadow root
  ...
  // get the last daikin-tile-title from shadow root
  ...
  // get div element from daikin-tile
  @query("div")
  private _divWrap: HTMLElement | null | undefined;

  /**
   * Specify whether the last of tile-title should show slash
   */
  ...
  /**
   * Specify overflow
   * when `visible` the tile will not be omitted even tile-titles total width exceed container width
   */
  ...
  _omit() {
    // remove items and add omission if daikin-tile is to long
    const divWidth = this._divWrap?.offsetWidth;
    const tileWidth = this.offsetWidth;
    ...
    if (
      ...
      (tileWidth > this.divOriginalWidth || !this._isEllipsis) &&
      this.omissionMode
    ) {
